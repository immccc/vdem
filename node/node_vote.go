...
func (node *Node) RequestShuffleVoteWithRandomPeer(pollId string) error {
	...
	peerKeys := make([]string, 0, len(poll.PeersWithVote))
	for peerKey := range poll.PeersWithVote {
	...
	event := messaging.SwapVotesEvent(peer.PubKey, peerKey, vote.EncodedSelection)
